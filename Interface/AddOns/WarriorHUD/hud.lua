...
function WHUD_CREATEFRAMES()
	...
	-- COOLDOWN BAR --
		...
		for i=1,6 do
			...
			CreateFrame("Frame", "WHUD_CDBAR"..i.."_SHINE", _G["WHUD_CDBAR"..i])
			_G["WHUD_CDBAR"..i.."_SHINE"]:SetAllPoints(_G["WHUD_CDBAR"..i])
...
function WHUD_UPDATE_SPELLINFO()
	...
	for i=1,100 do
		...
		if name == nil then return
			...
			-- Overpower/Revenge/Shield Bash/Pummel are no longer in the list and need to be checked extra
...
function WHUD_SetCooldown(name)
	if WHUD_INIT_ONCE and name then
...
WHUD_CORE:SetScript('OnEvent', function() 
	if event == "PLAYER_ENTERING_WORLD" then
		if WHUD_LOADED == nil then WHUD_INIT()
		else
			for i=1,6 do
				_G["WHUD_CDBAR"..i.."_SHINE"]:Hide()
			end
		end
	...
	elseif evnet == "PLAYER_DEAD" then
		-- player died, hide the ragebar
		WHUD_RAGE:SetVertexColor(0,0,0,0)
		WHUD_RAGE_TEXT:SetText("")
	...
	-- CREDITS TO ANARON (EHAUGW) @ Feenix (wow-one.com)
		if (GetUnitName("target")) then
			...
			elseif string.find(arg1,"Your Overpower") then
				WHUD_OP_TIME = 0 -- Hide the Overpower Alert if it's still visible
				WHUD_SetCooldown("Overpower")
			elseif string.find(arg1,"Your Revenge") then
				WHUD_SetCooldown("Revenge")
			elseif string.find(arg1,"Your Shield Bash") then
				WHUD_SetCooldown("Shield Bash")
			elseif string.find(arg1,"Your Pummel") then
				WHUD_SetCooldown("Pummel")
			end
		end
...
WHUD_CORE:SetScript('OnUpdate', function()
		...
		if WHUD_VARS.Cooldowns.enabled then
			...
			for i=1,6 do
				if _G["WHUD_CD"..i.."_USED"] ~= "" and _G["WHUD_CD"..i.."_TIME"] > 0 then
					if _G["WHUD_CD"..i.."_USED"] == "Overpower" and (WHUD_SPELLINFO[_G["WHUD_CD"..i.."_USED"]][2]+5) <=GetTime() and WHUD_OP_TIME > 0 then
						if WHUD_VARS.Overpower.mode == "text" then
					...
					if _G["WHUD_CD"..i.."_TIME"]+WHUD_VARS.Cooldowns.flashtime <= GetTime() then
						--WHUD_StartToShine(_G["WHUD_CDBAR"..i.."_SHINE"]) as of 1.8 we won't use the shine until it's 100% fixed
	...
	--
function WHUD_CDedit() -- will rework the entire editmode section in 1.9
...
local function CreateShineFrame(frame)
	local shineFrame = frame;
	shineFrame:SetAllPoints(frame:GetParent());
	...
	shineFrame:SetAlpha(frame:GetParent():GetAlpha());
	
	return shineFrame;
end
function WHUD_StartToShine(frame)
	shineFrame = CreateShineFrame(frame);
